...
function draw(ctx: CanvasRenderingContext2D, shape: Shape | TempShape) {
  if (!shape.points[0]) {
    console.error("Invalid points founded on draw!", shape);
    throw new Error("Invalid points founded on draw!");
  }
  ...
  if (shape.properties) {
    ...
    restoreCanvasState(ctx, {...state, ...(shape.properties ?? {})});
  }

  if (!shape.points[1]) {
    drawInitShape(ctx, toPoint(shape.points[0]));
  } else {
    switch (shape.type) {
      case "STRAIGHT_LINE":
        drawStraightLine(
        ...
        break;
      case "SQUARE_SHAPE": {
        drawRect(ctx, toPoint(shape.points[0]), toPoint(shape.points[1]));
        break;
      }
      case "CIRCLE_SHAPE": {
        drawCircle(ctx, toPoint(shape.points[0]), toPoint(shape.points[1]));
        break;
      }
      case "FREE_HAND": {
        ctx.beginPath();
        ...
        for (const point of shape.points) {
          drawFreeHand(ctx, toPoint(point));
        }
        break;
      }
    }
...
export {draw, redrawCanvasFromShapes};
